...
contract OBOSyrup is BEP20('OBOSyrup Token', 'OBOSYRUP') {
    ...
    function safeOboTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy Oboswap to OboSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to Oboswap LP tokens.
    // OboSwap must mint EXACTLY the same amount of OboSwap LP tokens or
    // else something bad will happen. Traditional Oboswap does not
...
contract MasterChef is Ownable {
  ...
  function pendingObo(uint256 _pid, address _user) external view returns (uint256) {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit OBO by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw Obo by unstaking');
    ...
    // Stake Obo tokens to MasterChef
    ...
    // Withdraw Obo tokens from STAKING.
